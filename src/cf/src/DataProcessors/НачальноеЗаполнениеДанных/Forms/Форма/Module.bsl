...
Процедура Подключаемый_ТабличныйДокументПриИзменении(Элемент)
	...
	Если ЭтоИзменениеЯчейки Или ЭтоИзменениеЗаголовка Тогда
		...
		РезультатПроверки = РезультатПроверкиДанныхЯчейки(ТекущаяОбласть, ЗначениеРеквизитаСтруктуры.ТипЗначения, ИмяРеквизита);
...
&НаСервере
Функция ЭтоМультиязычнаяКонфигурация()
...
Функция ПолучитьРезультатЗаполненияПоКоду()
	...
	Попытка
		Если ЭтоМультиязычнаяКонфигурация() Тогда
			// Необходимо для корректной работы процедуры Подключаемый_ЗаполнитьМультиязычныйРеквизит.
...
Процедура ВыполнитьВБезопасномРежиме(Знач Алгоритм, Знач Параметры = Неопределено)
	...
	// АПК:487-выкл Необходимо для корректной работы в мультиязычных конфигурациях (исполняемый код безопасен)
	Выполнить Алгоритм;
	// АПК:487-вкл
